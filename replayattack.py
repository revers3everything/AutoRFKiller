...
def record_signal(sample_rate, frequency, file_name):
    command = f"hackrf_transfer -s {sample_rate} -f {frequency} -r {file_name} -n 20000000"
    print(f"Executing: {command}")
    os.system(command)

def replay_signal(sample_rate, frequency, file_name):
    command = f"hackrf_transfer -s {sample_rate} -f {frequency} -t {file_name} -a 1 -x 24 -n 20000000"
    print(f"Executing: {command}")
    os.system(command)
